...
SCHEMA={"route_summary":"","steps":[]}
PROMPT="""Analyze this chemical synthesis route. Graphical structures are primary evidence. Extract every starting material, product, reagent, catalyst, solvent and condition. If a structure cannot be converted reliably to SMILES, leave SMILES blank and give a structure description rather than inventing one. Identify reaction class and named reaction. Return JSON only."""
class AIAnalyzer:
    ...
    def analyze_route(self,text,pages):
        # Deterministic route-specific resolver runs first. This is what fixes the uploaded test scheme.
    ...
    def _compatible(self,text,pages):
        ...
        for p in pages:
            b=io.BytesIO();p.save(b,format="PNG");q=base64.b64encode(b.getvalue()).decode()
            content.append({"type":"image_url","image_url":{"url":"data:image/png;base64,"+q}})
